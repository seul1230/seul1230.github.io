...
## 네이버 뉴스 기사 댓글 가져오기
### 1. URL 가져오기
뉴스 기사 댓글 부분으로 들어가면 해당 기사에 대한 댓글만 볼 수 있다. 이 페이지에서 마우스 우클릭 > 검사 (Inspect) > Network를 눌러 다음과 같은 창이 나오면 🚫 표시를 눌러 초기화해준다. <br/>
![initialize_log]() <br/><br/>
여기서 밑으로 스크롤을 내리면 보이는 `더보기` 버튼을 눌러주면 JS request가 새로 생긴 것을 볼 수 있다.<br/>
![comment_more]() <br/><br/>
url은 이 부분의 `Headers`에 있는 `Request URL`을 참고해서 가져올 것이다. <br/>
![headers_requesturl]() <br/><br/>

...
리스트는 index를 가지고 있는 게 아니라 검색은 느리지만, 연결된 node의 포인터 위치 정보를 가지고 있기 때문에 중간에 삽입 및 삭제가 일어날 때 전체를 돌지 않아도 이전 값과 다음값이 가르켰던 주소값만 수정하여 연결시켜주면 되기 때문에 빠르게 진행할 수 있다. 또한, array는 메모리 상에 데이터가 연속적으로 저장되지만 리스트는 메모리 상에 데이터가 비연속적으로 저장된다는 특징을 가지고 있다. 
...
<p class = 'callout'>left child index = (parent index) x 2

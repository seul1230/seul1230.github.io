...
포지셔널 인코딩은 각 토큰이 시퀀스 내에서 어느 위치에 있는지를 나타내는 벡터로, 토큰 임베딩에 더해져 모델에 입력된다. 즉, 트랜스포머는 &nbsp; <code>토큰의 의미 정보(Embedding) + 위치 정보(Positional Encoding)</code>를 함께 사용해 단어의 이미와 순서를 고려해 입력 표현을 구성한다.
